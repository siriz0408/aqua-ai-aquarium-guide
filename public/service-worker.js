...
async function showNotification(notificationData) {
  ...
  const options = {
    ...
    tag: `aqua-ai-${type}-${taskId || Date.now()}`,
    renotify: true,
...
self.addEventListener('notificationclick', (event) => {
  ...
  if (action === 'complete' && notificationData.taskId) {
    ...
    handleTaskComplete(notificationData.taskId);
  } else if (action === 'snooze' && notificationData.taskId) {
    // Snooze task for specified duration
    handleTaskSnooze(notificationData.taskId);
  } else if (action === 'view') {
    // Open the app to view details
    handleViewDetails(notificationData);
  } else if (action === 'dismiss') {
    // Just dismiss the notification (already closed)
...
async function handleTaskComplete(taskId) {
  try {
    ...
    // For now, we'll just post a message to any open clients
    ...
    console.log('Service Worker: Task completion requested', taskId);
  } catch (error) {
...
async function handleTaskSnooze(taskId) {
  try {
    ...
    console.log('Service Worker: Task snooze requested', taskId);
  } catch (error) {
...
async function handleViewDetails(notificationData) {
  try {
    // Determine the URL based on notification type
    let url = '/';
    
    if (notificationData.type === 'due_task' || notificationData.type === 'overdue_task') {
      url = '/maintenance-scheduler';
    } else if (notificationData.type === 'critical_alert') {
      url = '/log-parameters';
    } else if (notificationData.type === 'ai_insight') {
      url = '/aquabot';
    }
