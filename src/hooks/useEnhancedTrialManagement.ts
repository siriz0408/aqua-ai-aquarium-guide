...
import { validatePriceId, type PricingPlan } from '@/config/pricing';
...
export const useEnhancedTrialManagement = () => {
  ...
  const startDatabaseTrial = async (trialLengthDays: number = 3) => {
    ...
    if (trialLengthDays < 1 || trialLengthDays > 14) {
      const error = 'Trial length must be between 1 and 14 days.';
      setLastError(error);
      toast({
        title: "Invalid Trial Length",
        description: error,
        variant: "destructive",
      });
      return { success: false, error };
  ...
  const startStripeTrial = async (plan: PricingPlan) => {
    if (!user?.id) {
      const error = 'Please sign in to start your trial.';
      setLastError(error);
      toast({
    ...
    const validation = validatePriceId(plan.priceId);
    if (!validation.valid) {
      const error = validation.error || 'Invalid plan configuration';
      setLastError(error);
      toast({
