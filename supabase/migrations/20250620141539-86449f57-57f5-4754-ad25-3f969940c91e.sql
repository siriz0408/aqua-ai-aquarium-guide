...
-- Phase 1: Database Schema Updates for Subscription Plan Implementation

-- 1. Add comprehensive trial tracking and prevention columns
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS has_used_trial BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS trial_type VARCHAR(20) DEFAULT NULL, -- 'stripe' or 'database'
...
-- 2. Create subscription events tracking table
CREATE TABLE IF NOT EXISTS public.subscription_events (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE,
  event_type VARCHAR(50) NOT NULL, -- 'trial_started', 'trial_ended', 'subscription_created', 'subscription_cancelled', etc.
...
-- 3. Create comprehensive subscription access function
CREATE OR REPLACE FUNCTION public.check_user_subscription_access(user_id UUID)
RETURNS TABLE(
  has_access BOOLEAN,
  access_type TEXT, -- 'admin', 'paid', 'trial', 'trial_expired', 'free'
...
-- 4. Create trial start function with prevention logic
...
-- 5. Enhanced sync function for webhooks
...
-- 6. Admin function to extend trial
...
BEGIN
  ...
  new_trial_end := COALESCE(target_user.trial_end_date, NOW()) + INTERVAL '1 day' * extension_days;
  ...
  -- Log admin action
  INSERT INTO subscription_events (user_id, event_type, event_data)
  VALUES (target_user_id, 'trial_extended_by_admin', jsonb_build_object(
    'admin_user_id', admin_user_id,
    'extension_days', extension_days,
    'new_trial_end', new_trial_end,
    'previous_trial_end', target_user.trial_end_date
  ));
